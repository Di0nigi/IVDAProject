...
class Groq(Resource):
    def get(self, id):
        ...
        company = Company(**cursor)
        grq=GroqClient()
        resp = grq.generate_poem(company_name=company.name,prompt_file_path="D:\dionigi\Documents\IvdaProj\ivda-tutorial-webapp-main\example_project\hello-world\services\\backend\src\llm\prompts\groq_api_poem.json")
        return resp
class GroqInfo(Resource):
    def get(self,id):
        # retrieve the arguments and convert to a dict
        cursor = companies.find_one_or_404({"id": id})
        company = Company(**cursor)
        ...
        profitVal = args['selectedProfit']

        grq=GroqClient()
        resp = grq.generateCustom(company_name=company.name,profitVal=profitVal,year=year,prompt_file_path="D:\dionigi\Documents\IvdaProj\ivda-tutorial-webapp-main\example_project\hello-world\services\\backend\src\llm\prompts\groq_api_additional_information.json")
...
api.add_resource(Groq,'/poem/<int:id>')
api.add_resource(GroqInfo,'/info/<int:id>')